...
class Reappear2 extends JFrame implements ActionListener
{
...
Connection con;
PreparedStatement pst,pstup,pstdel;
String a;
Long rl;
String b;
...
JComboBox cb82;
JTextField t1,t2,t3,t4,t5,t6,t7,t8,tsouth;
JPanel north,center,south;
JLabel data;
JButton bmake,bsubmit,bupdate,bdelete;
...
rl=x;
		...
		center = new JPanel(new GridLayout(10,2,20,10));
...
tsouth = new JTextField(30);
		...
		center.add(bupdate);	center.add(bdelete);
// ADD ITEM LISTENERS
	...
	data = new JLabel("ROLL NO - "+Long.toString(rl));
	data.setFont(f);
	tsouth.setFont(f);
//north
		north.add(l);
...
{	a =b+" ";	}
	...
	if(ae.getSource() == bupdate)
	{
		try
		{
			pstup = con.prepareStatement("update REAPPEARS set reappear =? where roll_no=?");
			
			pstup.setString(1,a);
			pstup.setLong(2,rl);

			int update_var;
			update_var = pstup.executeUpdate();
			if(update_var > 0)
			{
				new JOptionPane().showMessageDialog(this,"SUCESSFUL UPDATE");
				clear();
				dispose();
			}
		}
		...
		{
			new JOptionPane().showMessageDialog(null,"Update : "+ex.getMessage());
		}
	}
		...
		if(ae.getSource() == bdelete)
		{
			...
			int jp_var;
			jp_var = conf.showConfirmDialog(this,"ARE YOU SURE YOU WANT TO DELETE ?");
			if(jp_var == conf.YES_OPTION)
			{
				try
				{
					pstdel = con.prepareStatement("delete from REAPPEARS where roll_no =?");
					pstdel.setLong(1,rl);
				
					int del_var;
					del_var = pstdel.executeUpdate();
					if(del_var > 0)
					{
					new JOptionPane().showMessageDialog(this,"DELETE SUCCESSFUL");
					clear();
					dispose();
					}
				}
				...
				{
					new JOptionPane().showMessageDialog(null,"Delete : "+ex.getMessage());
				}
			}	//end if
		}		//end bdelete
}	// end actionPerformed
...
public void getreap(Long lg)
{
	...
	try
	{
		...
		pest = con.prepareStatement("select * from REAPPEARS where roll_no="+lg);
		...
		{
			new JOptionPane().showMessageDialog(this,"NO RECORD");
...
public void clear()
{
	...
	cb12.setSelectedIndex(0);	cb22.setSelectedIndex(0);	cb32.setSelectedIndex(0);
	cb42.setSelectedIndex(0);	cb52.setSelectedIndex(0);	cb62.setSelectedIndex(0);
	cb72.setSelectedIndex(0);	cb82.setSelectedIndex(0);	
}
